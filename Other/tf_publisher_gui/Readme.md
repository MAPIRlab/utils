...
The resulting TF is by default published at 30Hz, but you can adjust the frequency with the `freq` param. Once you have correctly adjusted the transformation params, you can also run the node without the GUI by setting the `renderGUI` ROS param to `false`.